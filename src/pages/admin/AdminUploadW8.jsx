/**
 * AdminUploadW8 — 一次性上傳 W8 內容到 Firestore
 *
 * 沿用 AdminUploadW7 pattern(2026-05-23):
 *  - quiz 編輯介面(M2.5)還沒做,所以還是用 one-shot 頁面
 *  - Tiffany 進這個 URL 按一個按鈕就能上傳,不用碰程式碼或終端機
 *  - 上傳後可以從 /admin/content/8 編輯所有 metadata
 *
 * 上傳設定:
 *  - badgeId = 'B-Q1-W08' / cardId = 'C-Q1-W08'(內嵌 reward docs)
...
import { W8_WEEK_DATA, W8_BADGE_DATA, W8_CARD_DATA } from '@/data/W8-content';

export default function AdminUploadW8() {
  const navigate = useNavigate();
  const upsertMutation = useUpsertWeek();
  const { data: existingW8 } = useWeek(8);
  ...
  const alreadyExists = !!existingW8;
  ...
  const handleUpload = async () => {
    ...
    try {
      ...
      const badgeRef = doc(db, 'rewards', W8_BADGE_DATA.rewardId);
      await setDoc(
        badgeRef,
        { ...W8_BADGE_DATA, updatedAt: serverTimestamp(), createdAt: serverTimestamp() },
        { merge: true }
      );

      // 2. Upsert card to /rewards/{rewardId}
      const cardRef = doc(db, 'rewards', W8_CARD_DATA.rewardId);
      await setDoc(
        cardRef,
        { ...W8_CARD_DATA, updatedAt: serverTimestamp(), createdAt: serverTimestamp() },
        { merge: true }
      );
      ...
      console.error('W8 upload failed:', err);
      setStatus('error');
  ...
  return (
    <div className="space-y-4 max-w-3xl">
      <div className="flex items-center gap-3">
        ...
        <div>
          <h1 className="text-lg font-black text-slate-900">📦 上傳 Week 8</h1>
          <p className="text-xs text-slate-500">
            一次性工具 · 上傳完可以從 /admin/content/8 編輯
          </p>
        </div>
      </div>
      ...
      <div className="bg-white border border-slate-200 rounded-xl p-5 space-y-3">
        ...
        <dl className="text-sm text-slate-700 space-y-1.5">
          <Row label="標題" value={W8_WEEK_DATA.title} />
          <Row
            label="Quarter / Chapter"
            value={`Q${W8_WEEK_DATA.quarter} · ${W8_WEEK_DATA.chapter}`}
          />
          <Row label="大問題" value={W8_WEEK_DATA.question} />
          <Row label="練習題數" value={`${W8_WEEK_DATA.quizQuestions.length} 題`} />
          <Row label="任務標題" value={W8_WEEK_DATA.taskTitle} />
          <Row
            label="徽章"
            value={`${W8_BADGE_DATA.name}(${W8_BADGE_DATA.rewardId})`}
          />
          <Row
            label="卡片"
            value={`${W8_CARD_DATA.name}(${W8_CARD_DATA.rewardId})`}
          />
        ...
        <div className="grid grid-cols-2 gap-4 pt-3 border-t border-slate-100">
          <div>
            ...
            {!badgeImgError && (
              <img
                src={W8_BADGE_DATA.image}
                alt="W8 badge preview"
      ...
      {alreadyExists && status !== 'success' && (
        <div className="bg-amber-50 border border-amber-200 rounded-xl p-4 flex gap-3">
          ...
          <div className="text-sm text-amber-900">
            <p className="font-bold">Firestore 裡已經有 Week 8</p>
            <p className="mt-1 text-amber-800">
              按下「上傳」會覆蓋現有的 Week 8 內容(merge 模式,沒列在這頁的欄位會保留)。
              如果你已經在 /admin/content/8 改過東西,先去那邊確認。
            </p>
          </div>
        </div>
      )}
          ...
          >
            {status === 'uploading' ? (
              ...
              <>
                ...
                {alreadyExists ? '覆蓋 Week 8' : '上傳 Week 8 到 Firestore'}
              </>
            )}
          </button>
        )}
        ...
        {status === 'success' && (
          <div className="space-y-3">
            <div className="flex items-center gap-2 text-green-700">
              ...
              <p className="font-bold">Week 8 已上傳 ✓</p>
            </div>
            ...
            <ol className="text-sm text-slate-700 list-decimal pl-5 space-y-1">
              <li>
                到 <code className="bg-slate-100 px-1.5 py-0.5 rounded text-xs">/admin/content/8</code>{' '}
                檢查內容、改文字、確認 badge/card 都接上了
              </li>
              <li>
                準備好上線時:在 /admin/content/8 把「published」打開,並到{' '}
                <code className="bg-slate-100 px-1.5 py-0.5 rounded text-xs">brandSettings/main</code>{' '}
                把 latestAvailableWeek 改成 8(目前 = 7)
              </li>
              <li>
                到 Home 確認 W8 卡片有出現、能點進去、quiz + task 正常
              </li>
            </ol>
            <button
              onClick={() => navigate('/admin/content/8')}
              className="w-full mt-2 bg-slate-900 text-white font-bold px-4 py-3 rounded-lg hover:opacity-90 transition-all"
            >
              去 /admin/content/8 編輯
            </button>
          </div>
        )}
      ...
      <details className="bg-white border border-slate-200 rounded-xl">
        ...
        <div className="px-5 pb-4 space-y-3 border-t border-slate-100 pt-4">
          {W8_WEEK_DATA.quizQuestions.map((q) => (
