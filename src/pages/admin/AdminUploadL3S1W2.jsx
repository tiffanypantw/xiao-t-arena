/**
 * AdminUploadL3S1W2 — 一次性上傳「投資與理財規劃(第一季)」W2 到 Firestore
 *
 * 沿用 AdminUploadL3S1W1 / AdminUploadL2S1W2 pattern(2026-07-28):
 *  - Tiffany 進 /admin/upload-l3s1-w2 按一個按鈕就能上傳,不用碰程式碼
 *  - 上傳後可以從 /admin/content/302 編輯 metadata、發佈
 *  - 這一週掛在新學習道路主題(l3-s1,L3 資本 band)上,系統內部 weekNumber = 302
 *
 * 上傳設定:
 *  - badgeId = 'B-L3S1-W02' / cardId = 'C-L3S1-W02'
 *  - published = false(草稿,發佈後道路上的 W2 才會亮起)
...
import { L3S1_W2_WEEK_DATA, L3S1_W2_BADGE_DATA, L3S1_W2_CARD_DATA } from '@/data/L3S1-W2-content';
...
export default function AdminUploadL3S1W2() {
  const navigate = useNavigate();
  const upsertMutation = useUpsertWeek();
  const { data: existing } = useWeek(302);
  ...
  const handleUpload = async () => {
    ...
    try {
      ...
      const badgeRef = doc(db, 'rewards', L3S1_W2_BADGE_DATA.rewardId);
      await setDoc(
        badgeRef,
        { ...L3S1_W2_BADGE_DATA, updatedAt: serverTimestamp(), createdAt: serverTimestamp() },
        { merge: true }
      );

      // 2. Upsert card to /rewards/{rewardId}
      const cardRef = doc(db, 'rewards', L3S1_W2_CARD_DATA.rewardId);
      await setDoc(
        cardRef,
        { ...L3S1_W2_CARD_DATA, updatedAt: serverTimestamp(), createdAt: serverTimestamp() },
        { merge: true }
      );
      ...
      await upsertMutation.mutateAsync({
        weekNumber: 302,
        data: L3S1_W2_WEEK_DATA,
        isNew: !alreadyExists,
      });
      setStatus('success');
    } catch (err) {
      console.error('L3S1-W2 upload failed:', err);
      setStatus('error');
  ...
  return (
    <div className="space-y-4 max-w-3xl">
      <div className="flex items-center gap-3">
        ...
        <div>
          <h1 className="text-lg font-black text-slate-900">📦 上傳「投資與理財規劃」第一季 W2</h1>
          <p className="text-xs text-slate-500">
            一次性工具 · 學習道路第 2 關(系統編號 week 302) · 上傳完可從 /admin/content/302 編輯
          </p>
        </div>
      </div>
      ...
      <div className="bg-white border border-slate-200 rounded-xl p-5 space-y-3">
        ...
        <dl className="text-sm text-slate-700 space-y-1.5">
          <Row label="標題" value={L3S1_W2_WEEK_DATA.title} />
          <Row label="主題" value="學習道路 · 投資與理財規劃(第一季) · 第 2 關" />
          <Row label="大問題" value={L3S1_W2_WEEK_DATA.question} />
          <Row
            label="練習題數"
            value={`${L3S1_W2_WEEK_DATA.quizQuestions.length} 題(全選擇題)+ 論證題 1 題(老師審核)`}
          />
          <Row label="任務標題" value={L3S1_W2_WEEK_DATA.taskTitle} />
          <Row
            label="徽章"
            value={`${L3S1_W2_BADGE_DATA.name}(${L3S1_W2_BADGE_DATA.rewardId})`}
          />
          <Row
            label="卡片"
            value={`${L3S1_W2_CARD_DATA.name}(${L3S1_W2_CARD_DATA.rewardId})`}
          />
          <Row label="發佈狀態" value="草稿(道路上的 W2 還不會亮,發佈後才開放)" />
        </dl>
        ...
        <div className="grid grid-cols-2 gap-4 pt-3 border-t border-slate-100">
          <div>
            ...
            <code className="text-[10px] bg-slate-100 px-2 py-1 rounded block break-all">
              {L3S1_W2_BADGE_DATA.image}
            </code>
            {!badgeImgError && (
              <img
                src={L3S1_W2_BADGE_DATA.image}
                alt="L3S1 W2 badge preview"
          ...
          <div>
            ...
            <code className="text-[10px] bg-slate-100 px-2 py-1 rounded block break-all">
              {L3S1_W2_CARD_DATA.image}
            </code>
            {!cardImgError && (
              <img
                src={L3S1_W2_CARD_DATA.image}
                alt="L3S1 W2 card preview"
      ...
      {alreadyExists && status !== 'success' && (
        <div className="bg-amber-50 border border-amber-200 rounded-xl p-4 flex gap-3">
          ...
          <div className="text-sm text-amber-900">
            <p className="font-bold">Firestore 裡已經有 week 302</p>
            <p className="mt-1 text-amber-800">
              按下「上傳」會覆蓋現有內容(merge 模式,沒列在這頁的欄位會保留)。
              如果你已經在 /admin/content/302 改過東西,先去那邊確認。
            </p>
          </div>
        </div>
      )}
          ...
          >
            {status === 'uploading' ? (
              ...
              <>
                ...
                {alreadyExists ? '覆蓋 W2(week 302)' : '上傳 W2 到 Firestore'}
              </>
            )}
          </button>
        )}
      ...
      <details className="bg-white border border-slate-200 rounded-xl">
        ...
        <div className="px-5 pb-4 space-y-3 border-t border-slate-100 pt-4">
          {L3S1_W2_WEEK_DATA.quizQuestions.map((q) => (
          ...
          <div className="text-sm pt-1">
            <div className="text-xs font-bold text-slate-500 mb-1">✍️ 論證題(評鑑層 · 老師後台審核 → 徽章)</div>
            <div className="text-slate-900 bg-amber-50 border border-amber-100 rounded p-2.5 leading-relaxed whitespace-pre-line">
              {L3S1_W2_WEEK_DATA.openQuestion}
            </div>
          </div>
        </div>
      </details>
