/**
 * AdminUploadL3S1W1 — 一次性上傳「投資與理財規劃(第一季)」W1 到 Firestore
 *
 * 沿用 AdminUploadL1S1W1 / AdminUploadL2S1W1 pattern(2026-07-11):
 *  - Tiffany 進 /admin/upload-l3s1-w1 按一個按鈕就能上傳,不用碰程式碼
 *  - 上傳後可以從 /admin/content/301 編輯 metadata、發佈
 *  - 這一週掛在新學習道路主題(l3-s1,L3 資本 band)上,系統內部 weekNumber = 301
 *
 * 上傳設定:
 *  - badgeId = 'B-L3S1-W01' / cardId = 'C-L3S1-W01'
...
import { L3S1_W1_WEEK_DATA, L3S1_W1_BADGE_DATA, L3S1_W1_CARD_DATA } from '@/data/L3S1-W1-content';
...
export default function AdminUploadL3S1W1() {
  const navigate = useNavigate();
  const upsertMutation = useUpsertWeek();
  const { data: existing } = useWeek(301);
  ...
  const handleUpload = async () => {
    ...
    try {
      ...
      const badgeRef = doc(db, 'rewards', L3S1_W1_BADGE_DATA.rewardId);
      await setDoc(
        badgeRef,
        { ...L3S1_W1_BADGE_DATA, updatedAt: serverTimestamp(), createdAt: serverTimestamp() },
        { merge: true }
      );

      // 2. Upsert card to /rewards/{rewardId}
      const cardRef = doc(db, 'rewards', L3S1_W1_CARD_DATA.rewardId);
      await setDoc(
        cardRef,
        { ...L3S1_W1_CARD_DATA, updatedAt: serverTimestamp(), createdAt: serverTimestamp() },
        { merge: true }
      );
      ...
      await upsertMutation.mutateAsync({
        weekNumber: 301,
        data: L3S1_W1_WEEK_DATA,
        isNew: !alreadyExists,
      });
      setStatus('success');
    } catch (err) {
      console.error('L3S1-W1 upload failed:', err);
      setStatus('error');
  ...
  return (
    <div className="space-y-4 max-w-3xl">
      <div className="flex items-center gap-3">
        ...
        <div>
          <h1 className="text-lg font-black text-slate-900">📦 上傳「投資與理財規劃」第一季 W1</h1>
          <p className="text-xs text-slate-500">
            一次性工具 · 學習道路第 1 關(系統編號 week 301) · 上傳完可從 /admin/content/301 編輯
          </p>
        </div>
      </div>
      ...
      <div className="bg-white border border-slate-200 rounded-xl p-5 space-y-3">
        ...
        <dl className="text-sm text-slate-700 space-y-1.5">
          <Row label="標題" value={L3S1_W1_WEEK_DATA.title} />
          <Row label="主題" value="學習道路 · 投資與理財規劃(第一季) · 第 1 關" />
          <Row label="大問題" value={L3S1_W1_WEEK_DATA.question} />
          <Row
            label="練習題數"
            value={`${L3S1_W1_WEEK_DATA.quizQuestions.length} 題(全選擇題)+ 論證題 1 題(老師審核)`}
          />
          <Row label="任務標題" value={L3S1_W1_WEEK_DATA.taskTitle} />
          <Row
            label="徽章"
            value={`${L3S1_W1_BADGE_DATA.name}(${L3S1_W1_BADGE_DATA.rewardId})`}
          />
          <Row
            label="卡片"
            value={`${L3S1_W1_CARD_DATA.name}(${L3S1_W1_CARD_DATA.rewardId})`}
          />
        ...
        <div className="grid grid-cols-2 gap-4 pt-3 border-t border-slate-100">
          <div>
            ...
            <code className="text-[10px] bg-slate-100 px-2 py-1 rounded block break-all">
              {L3S1_W1_BADGE_DATA.image}
            </code>
            {!badgeImgError && (
              <img
                src={L3S1_W1_BADGE_DATA.image}
                alt="L3S1 W1 badge preview"
          ...
          <div>
            ...
            <code className="text-[10px] bg-slate-100 px-2 py-1 rounded block break-all">
              {L3S1_W1_CARD_DATA.image}
            </code>
            {!cardImgError && (
              <img
                src={L3S1_W1_CARD_DATA.image}
                alt="L3S1 W1 card preview"
      ...
      {alreadyExists && status !== 'success' && (
        <div className="bg-amber-50 border border-amber-200 rounded-xl p-4 flex gap-3">
          ...
          <div className="text-sm text-amber-900">
            <p className="font-bold">Firestore 裡已經有 week 301</p>
            <p className="mt-1 text-amber-800">
              按下「上傳」會覆蓋現有內容(merge 模式,沒列在這頁的欄位會保留)。
              如果你已經在 /admin/content/301 改過東西,先去那邊確認。
            </p>
          </div>
        </div>
      )}
          ...
          >
            {status === 'uploading' ? (
              ...
              <>
                ...
                {alreadyExists ? '覆蓋 W1(week 301)' : '上傳 W1 到 Firestore'}
              </>
            )}
          </button>
        )}
        ...
        {status === 'success' && (
          <div className="space-y-3">
            <div className="flex items-center gap-2 text-green-700">
              ...
              <p className="font-bold">「投資與理財規劃」W1 已上傳 ✓</p>
            </div>
            <p className="text-sm text-slate-600">下一步:</p>
            <ol className="text-sm text-slate-700 list-decimal pl-5 space-y-1">
              <li>
                到 <code className="bg-slate-100 px-1.5 py-0.5 rounded text-xs">/admin/content/301</code>{' '}
                檢查內容、改文字、確認 badge/card 都接上了
              </li>
              <li>
                準備好上線時:在 /admin/content/301 把「published」打開 —— 學習道路上的 W1 就會亮起
              </li>
              <li>
                到 概念競技場 → L3 資本 → 投資與理財規劃 → 點 W1,確認 10 題 + 論證題 + 任務都正常
              </li>
              <li>
      ...
      <details className="bg-white border border-slate-200 rounded-xl">
        ...
        <div className="px-5 pb-4 space-y-3 border-t border-slate-100 pt-4">
          {L3S1_W1_WEEK_DATA.quizQuestions.map((q) => (
          ...
          <div className="text-sm pt-1">
            ...
            <div className="text-slate-900 bg-amber-50 border border-amber-100 rounded p-2.5 leading-relaxed whitespace-pre-line">
              {L3S1_W1_WEEK_DATA.openQuestion}
            </div>
          </div>
        </div>
      </details>
