/**
 * AdminUploadL2S1W3 — 一次性上傳「商業與市場(第一季)」W3 到 Firestore
 *
 * 沿用 AdminUploadL2S1W1/W2 pattern:
 *  - Tiffany 進 /admin/upload-l2s1-w3 按一個按鈕就能上傳,不用碰程式碼
 *  - 上傳後可以從 /admin/content/203 編輯 metadata、發佈
 *  - 這一週掛在新學習道路主題(l2-s1,L2 創造 band)上,系統內部 weekNumber = 203
 *
 * 上傳設定:
 *  - badgeId = 'B-L2Q1-W03' / cardId = 'C-L2Q1-W03'
 *  - published = false(草稿,發佈後道路上的 W3 才會亮起)
...
import { L2S1_W3_WEEK_DATA, L2S1_W3_BADGE_DATA, L2S1_W3_CARD_DATA } from '@/data/L2S1-W3-content';
...
export default function AdminUploadL2S1W3() {
  const navigate = useNavigate();
  const upsertMutation = useUpsertWeek();
  const { data: existing } = useWeek(203);
  ...
  const handleUpload = async () => {
    ...
    try {
      ...
      const badgeRef = doc(db, 'rewards', L2S1_W3_BADGE_DATA.rewardId);
      await setDoc(
        badgeRef,
        { ...L2S1_W3_BADGE_DATA, updatedAt: serverTimestamp(), createdAt: serverTimestamp() },
        { merge: true }
      );

      // 2. Upsert card to /rewards/{rewardId}
      const cardRef = doc(db, 'rewards', L2S1_W3_CARD_DATA.rewardId);
      await setDoc(
        cardRef,
        { ...L2S1_W3_CARD_DATA, updatedAt: serverTimestamp(), createdAt: serverTimestamp() },
        { merge: true }
      );
      ...
      await upsertMutation.mutateAsync({
        weekNumber: 203,
        data: L2S1_W3_WEEK_DATA,
        isNew: !alreadyExists,
      });
      setStatus('success');
    } catch (err) {
      console.error('L2S1-W3 upload failed:', err);
      setStatus('error');
  ...
  return (
    <div className="space-y-4 max-w-3xl">
      <div className="flex items-center gap-3">
        ...
        <div>
          <h1 className="text-lg font-black text-slate-900">📦 上傳「商業與市場」第一季 W3</h1>
          <p className="text-xs text-slate-500">
            一次性工具 · 學習道路第 3 關(系統編號 week 203) · 上傳完可從 /admin/content/203 編輯
          </p>
        </div>
      </div>
      ...
      <div className="bg-white border border-slate-200 rounded-xl p-5 space-y-3">
        ...
        <dl className="text-sm text-slate-700 space-y-1.5">
          <Row label="標題" value={L2S1_W3_WEEK_DATA.title} />
          <Row label="主題" value="學習道路 · 商業與市場(第一季) · 第 3 關 市場氣象員" />
          <Row label="大問題" value={L2S1_W3_WEEK_DATA.question} />
          <Row
            label="練習題數"
            value={`${L2S1_W3_WEEK_DATA.quizQuestions.length} 題(選擇 9 + 是非 1)+ 開放題 1 題(老師審核)`}
          />
          <Row label="任務標題" value={L2S1_W3_WEEK_DATA.taskTitle} />
          <Row
            label="徽章"
            value={`${L2S1_W3_BADGE_DATA.name}(${L2S1_W3_BADGE_DATA.rewardId})`}
          />
          <Row
            label="卡片"
            value={`${L2S1_W3_CARD_DATA.name}(${L2S1_W3_CARD_DATA.rewardId})`}
          />
          <Row label="發佈狀態" value="草稿(道路上的 W3 還不會亮,發佈後才開放)" />
        </dl>
        ...
        <div className="grid grid-cols-2 gap-4 pt-3 border-t border-slate-100">
          <div>
            ...
            <code className="text-[10px] bg-slate-100 px-2 py-1 rounded block break-all">
              {L2S1_W3_BADGE_DATA.image}
            </code>
            {!badgeImgError && (
              <img
                src={L2S1_W3_BADGE_DATA.image}
                alt="L2S1 W3 badge preview"
          ...
          <div>
            ...
            <code className="text-[10px] bg-slate-100 px-2 py-1 rounded block break-all">
              {L2S1_W3_CARD_DATA.image}
            </code>
            {!cardImgError && (
              <img
                src={L2S1_W3_CARD_DATA.image}
                alt="L2S1 W3 card preview"
      ...
      {alreadyExists && status !== 'success' && (
        <div className="bg-amber-50 border border-amber-200 rounded-xl p-4 flex gap-3">
          ...
          <div className="text-sm text-amber-900">
            <p className="font-bold">Firestore 裡已經有 week 203</p>
            <p className="mt-1 text-amber-800">
              按下「上傳」會覆蓋現有內容(merge 模式,沒列在這頁的欄位會保留)。
              如果你已經在 /admin/content/203 改過東西,先去那邊確認。
            </p>
          </div>
        </div>
      )}
          ...
          >
            {status === 'uploading' ? (
              ...
              <>
                ...
                {alreadyExists ? '覆蓋 W3(week 203)' : '上傳 W3 到 Firestore'}
              </>
            )}
          </button>
        )}
        ...
        {status === 'success' && (
          <div className="space-y-3">
            <div className="flex items-center gap-2 text-green-700">
              ...
              <p className="font-bold">「商業與市場」W3 已上傳 ✓</p>
            </div>
            <p className="text-sm text-slate-600">下一步:</p>
            <ol className="text-sm text-slate-700 list-decimal pl-5 space-y-1">
              <li>
                到 <code className="bg-slate-100 px-1.5 py-0.5 rounded text-xs">/admin/content/203</code>{' '}
                檢查內容、改文字、確認 badge/card 都接上了
              </li>
              <li>
                準備好上線時:在 /admin/content/203 把「published」打開 —— 學習道路上的 W3 就會亮起
              </li>
              <li>
                到 概念競技場 → L2 創造 → 商業與市場 → 點 W3,確認 10 題 + 開放題 + 任務都正常
              </li>
              <li>
                開通碼不用重發 —— L2S1 的開通碼是主題層級的,W1 就開通過的孩子直接看得到 W3
              </li>
            </ol>
            <button
              onClick={() => navigate('/admin/content/203')}
              className="w-full mt-2 bg-slate-900 text-white font-bold px-4 py-3 rounded-lg hover:opacity-90 transition-all"
            >
              去 /admin/content/203 編輯
            </button>
          </div>
        )}
      ...
      <details className="bg-white border border-slate-200 rounded-xl">
        ...
        <div className="px-5 pb-4 space-y-3 border-t border-slate-100 pt-4">
          {L2S1_W3_WEEK_DATA.quizQuestions.map((q) => (
          ...
          <div className="text-sm pt-1">
            <div className="text-xs font-bold text-slate-500 mb-1">✍️ 開放題(評鑑層 · 老師後台審核 → 徽章)</div>
            <div className="text-slate-900 bg-amber-50 border border-amber-100 rounded p-2.5 leading-relaxed">
              {L2S1_W3_WEEK_DATA.openQuestion}
            </div>
          </div>
        </div>
      </details>
