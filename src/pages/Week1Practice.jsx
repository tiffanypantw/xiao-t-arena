...
function QuestionCard({ q, onAnswer, answered }) {
  ...
  const handleConfirm = () => {
    ...
    setRevealed(true);
    const isCorrect =
      q.type === 'true_false' ? selected === q.answer : selected === q.answer;
  ...
  if (q.type === 'true_false') {
    ...
    return (
      <CardShell q={q}>
          ...
          {options.map(opt => {
            ...
            return (
              <button
                ...
                className={`flex-1 rounded-xl border-2 py-4 text-base font-bold transition-all ${style}`}
              >
                {opt.label}
              </button>
            );
        ...
        {revealed && <FeedbackRow correct={selected === q.answer} correctLabel={q.answer ? '正確（○）' : '錯誤（✕）'} />}
      </CardShell>
    );
  }
...
// ── 連連看 元件 ──────────────────────────────────────
function MatchingQuestion({ onAnswer }) {
  const rights = [...MATCHING_PAIRS.map(p => p.right)].sort(() => Math.random() - 0.5);
  ...
  const allDone = Object.keys(selections).length === MATCHING_PAIRS.length;

  const handleConfirm = () => {
    setRevealed(true);
    const correctCount = MATCHING_PAIRS.filter(p => selections[p.left] === p.right).length;
    setTimeout(() => onAnswer(correctCount === MATCHING_PAIRS.length), 1200);
  };
  ...
  return (
    ...
    >
      <div className="flex items-center gap-2 mb-1">
        <span className="text-xs bg-muted text-muted-foreground px-2 py-0.5 rounded-full font-semibold">連連看</span>
        <span className="text-xs text-muted-foreground">Q9</span>
      </div>
      <p className="text-base font-bold text-foreground leading-snug">
        將左邊的情況和右邊的結果配對
      </p>
      {activeLeft && !revealed && (
        <p className="text-xs text-primary font-semibold">選取中：「{activeLeft}」→ 請點右邊的結果</p>
      )}
      ...
      <div className="grid grid-cols-2 gap-3">
        ...
        <div className="space-y-2">
          {MATCHING_PAIRS.map(p => {
            ...
            const isPaired = !!selections[p.left];
            const correctPair = MATCHING_PAIRS.find(x => x.left === p.left);
            const isCorrect = revealed && selections[p.left] === correctPair.right;
            const isWrong = revealed && isPaired && !isCorrect;
            return (
              <button
                key={p.left}
                onClick={() => handleLeftClick(p.left)}
                className={`w-full text-left rounded-xl border-2 px-3 py-3 text-xs font-semibold transition-all leading-snug ${
                  revealed
                    ? isCorrect ? 'border-green-500 bg-green-50 text-green-800'
                    : isWrong ? 'border-red-400 bg-red-50 text-red-700'
                    : 'border-border bg-card text-foreground'
                  : isActive ? 'border-primary bg-primary/5 text-primary'
                  : isPaired ? 'border-foreground bg-foreground text-background'
                  : 'border-border bg-card text-foreground hover:border-foreground/40'
                }`}
              >
                {p.left}
                {isPaired && !revealed && (
                  <span className="block text-[10px] opacity-70 mt-0.5">→ {selections[p.left]}</span>
                )}
              </button>
            );
          })}
        ...
        <div className="space-y-2">
          {shuffled.map(r => {
            const isUsed = Object.values(selections).includes(r);
            const correctLeft = MATCHING_PAIRS.find(p => p.right === r)?.left;
            const isCorrect = revealed && selections[correctLeft] === r;
            const isWrong = revealed && isUsed && !isCorrect;
            return (
              <button
                key={r}
                ...
                className={`w-full text-left rounded-xl border-2 px-3 py-3 text-xs font-semibold transition-all leading-snug ${
                  ...
                  : isUsed ? 'border-border bg-muted text-muted-foreground opacity-50'
                  : activeLeft ? 'border-secondary bg-secondary/5 text-foreground hover:border-secondary'
                  : 'border-border bg-card text-foreground'
                }`}
              >
                {r}
...
function ResultScreen({ correct, total, onRetry }) {
  ...
  return (
    ...
    >
      ...
      <Button onClick={onRetry} className="w-full text-base py-5">
        🔁 重新挑戰
      </Button>
    </motion.div>
  );
